...
contract MarketPlace {
  ...
  /// @param u Underlying token address associated with the market
  /// @param m Maturity timestamp of the market
  /// @param t Address of the user redeeming zcTokens
  /// @param a Amount of zcTokens being redeemed
  function redeemZcToken(address u, uint256 m, address t, uint256 a) public onlySwivel(swivel) returns (uint256) {
    ...
    if (!mature[u][m]) {
      ...
      require(ZcToken(mkt.zcTokenAddr).burn(t, a), 'could not burn'); // Burn user's zcTokens
  ...
  /// @param u Underlying token address associated with the market
  /// @param m Maturity timestamp of the market
  /// @param t Address of the user redeeming interest
  function redeemVaultInterest(address u, uint256 m, address t) public onlySwivel(swivel) returns (uint256) {
    // Call to the floating market contract to calculate the interest generated
    uint256 interestGenerated = VaultTracker(markets[u][m].vaultAddr).redeemInterest(t);
