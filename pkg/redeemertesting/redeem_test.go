...
func (s *redeemTestSuite) TestAPWineRedeem() {
	...
	assert.NoError(err)
	assert.Equal(s.Dep.LenderAddress, vaultCalled)
...
func (s *redeemTestSuite) TestTempusRedeem() {
	...
	balanceOfCalled, err := s.TempusToken.BalanceOfCalled()
	assert.NoError(err)
	assert.Equal(s.Dep.LenderAddress, balanceOfCalled)
...
func (s *redeemTestSuite) TestIlluminateRedeem() {
	...
	balanceOfCalled, err := s.ZcToken.BalanceOfCalled()
	assert.NoError(err)
	assert.Equal(s.Dep.LenderAddress, balanceOfCalled)
...
func (s *redeemTestSuite) TestPendleRedeem() {
	...
	balanceOfCalled, err := s.PendleToken.BalanceOfCalled()
	assert.NoError(err)
	assert.Equal(s.Dep.LenderAddress, balanceOfCalled)
...
func (s *redeemTestSuite) TestElementRedeem() {
	...
	balanceOfCalled, err := s.ElementToken.BalanceOfCalled()
	assert.NoError(err)
	assert.Equal(s.Dep.LenderAddress, balanceOfCalled)
...
func (s *redeemTestSuite) TestYieldRedeem() {
	...
	balanceOfCalled, err := s.YieldToken.BalanceOfCalled()
	assert.NoError(err)
	assert.Equal(s.Dep.LenderAddress, balanceOfCalled)
...
func (s *redeemTestSuite) TestNotionalRedeem() {
	...
	balanceOfCalled, err := s.Notional.BalanceOfCalled()
	assert.NoError(err)
	assert.Equal(s.Dep.LenderAddress, balanceOfCalled)
