...
contract Swivel {
  ...
  /// @dev This functionality is an abstraction used by ``, `` and ``. Note that while there is an extarnal method
  /// `withdraw` also on this contract the unique method signatures (and visibility) exclude any possible clashing
  /// @param p Protocol Enum Value
  /// @param c Compounding token address
